...
foreach ($video in $Videos) {
    Write-Host "=============================="
    Write-Host "Processing file: $video"
    Write-Host "==============================\`n"
    ...
    # نحدد الصيغة النهائية لنفس صيغة الملف الأصلي
    $vocals_final = Join-Path $dir "$name\`_vocals$ext"

    # لو الملف صوتي
    if ($ext -in ".mp3", ".wav", ".flac", ".m4a") {

        $vocals_wav = Join-Path $dir "separatedhtdemucs\$name\vocals.wav"

        Write-Host "=============================="
        Write-Host "1) Remove music from audio..."
        Write-Host "==============================\`n"
        ...
        Write-Host "=============================="
        Write-Host "2) Convert vocals to original format..."
        Write-Host "==============================\`n"

        ffmpeg -y -i "$vocals_wav" "$vocals_final"

        Write-Host "\`n✅ Done: $vocals_final\`n"
        ...
        $sep_dir = Join-Path $dir "separatedhtdemucs"
